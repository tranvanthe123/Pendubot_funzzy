...
beta1=0.01122;beta2=0.00193;beta3=0.00156;beta4=0.0406;beta5=0.0078;
g=9.81;
q1_init=pi/2+0.01;
q1_dot_init=-0.02;
q2_init=0.01;
...
max_generation=200000; %so the he toi da trong qua trinh chay(chay toi toi da 200 the he thi dung lai)
max_stall_generation=50000;
...
range=[ 0   0        0  0      0 0  ;...
        100  100    100 100  10 10 ]; %tam cua cac NST
dec=[2 2   2  2 1 1];  %vi tri dau cham thap phan
sig=[3 3   3  3 3 3];  %so chu so co nghia trong moi nhiem sac the
...
for pop_index=1:pop_size,                                                       %Khoi tao 
    K1=par(pop_index,1);                                                       %cac the he                                               %cha me
    K2=par(pop_index,2);                                                       %dau tien
    K3=par(pop_index,3);                                                       %(cho chay GA truoc                                         %de ra duoc cac cha me toi uu
    K4=par(pop_index,4);
    K5=par(pop_index,5);                                                       %cac the he                                               %cha me
    K6=par(pop_index,6);                                                       %dau tien
    sim('mophong_fuzzy.mdl');
    %(tuc l cho lai ghep thu truoc khi
...
[bestfit,bestchrom]=max(fitness);                                              %
    ...
    K10=par(bestchrom,1);                                                       %cac the he                                               %cha me
    K20=par(bestchrom,2);                                                       %dau tien
    K30=par(bestchrom,3);                                                       %(cho chay GA truoc                                         %de ra duoc cac cha me toi uu
    K40=par(bestchrom,4);
    K50=par(bestchrom,5);                                                       %cac the he                                               %cha me
    K60=par(bestchrom,6);                                                       %dau tien
...
while ~Terminal, %Terminal se bang 1 neu chay du 200 the he hoac trong qua trinh chay co 1 the he con cai có bestfit thoa man sai so <epsilon
    ...
    for pop_index=1:pop_size,                                           %lan luot test tung ca the trong quan the
            ...
            K6=par(pop_index,6);                                                       %dau tien
                                        %
        sim('mophong_fuzzy.mdl');                                             %tien hanh chay mo phong de kiem tra xem ca the nao toi uu
        J=(e1'*e1)+(e2'*e2);%+rho*(u'*u); 
       % fitness(pop_index)=1/(J+eps);
        if (length(e1)>9500)&&(J<Jmin)                                                                          %neu mo phong chay duoc du 10000 mau(100s) thi
            clc
            Jmin=J
            K1
            K2
