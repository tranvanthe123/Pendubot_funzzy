...
max_generation=2000000; %so the he toi da trong qua trinh chay(chay toi toi da 200 the he thi dung lai)
max_stall_generation=5000000;
epsilon=0.0001; %J chuan(neu the he nao co J<=epsilon tuc la da tim duoc thog so thoa man, GA khong can chay tiep nua)
pop_size=40; % so ca the trong qun the(so cha me)
npar = 6  %co 6 nhiem sac the trong 1 ca the(6 nhiem sac the nay lan luot la Kp1,Ki1, Kd1, Kp2,Ki2,Kd2
range=[ 0 0   0    0  0 0   ;...
        10 10  10   10 10 10 ]; %tam cua cac NST
dec=[1 1 1  1 1 1 ];  %vi tri dau cham thap phan
sig=[4 4 4  4 4 4 ];  %so chu so co nghia trong moi nhiem sac the

cross_prob = 0.3; %he so lai ghep
mutate_prob = 0.7; %he so dot bien(he so dot bien + he so lai ghep =1)
...
for pop_index=1:pop_size,                                                       %Khoi tao 
    a1=par(pop_index,1);                                                       %cac the he 
    a2=par(pop_index,2);                                                       %dau tien
    c1=par(pop_index,3);                                                       %(cho chay GA truoc
    c2=par(pop_index,4);    
    k2=par(pop_index,5);                                                       %(cho chay GA truoc
    nuy2=par(pop_index,6);    
    sim('pendubot_simulate_1_8_5_2017.mdl');                                                            %(tuc l cho lai ghep thu truoc khi
...
[bestfit,bestchrom]=max(fitness);                                              %
    ...
    a1=par(bestchrom,1);                                                       %cac the he 
    a2=par(bestchrom,2);                                                       %dau tien
    c1=par(bestchrom,3);                                                       %(cho chay GA truoc
    c2=par(bestchrom,4);    
    k2=par(bestchrom,5);                                                       %(cho chay GA truoc
    nuy2=par(bestchrom,6); 

J0=1/bestfit+0.001;                                  %(do elitism = nen doi hoi phai co 1 cha me tot nhat de so sanh voi cac he he sau, tim toi uu nhat)
J_min=20000000000000000000;
while ~Terminal, %Terminal se bang 1 neu chay du 200 the he hoac trong qua trinh chay co 1 the he con cai có bestfit thoa man sai so <epsilon
    ...
    nuy2=par(pop_index,6);    
                   %
        sim('pendubot_simulate_1_8_5_2017.mdl');                                             %tien hanh chay mo phong de kiem tra xem ca the nao toi uu
       % J=(e1'*e1)+(e2'*e2);%+rho*(u'*u); 
       % fitness(pop_index)=1/(J+eps);
       J=e1'*e1+e2'*e2; 
       if ((length(e1)>9500)&&(J<J_min))                                                                          %neu mo phong chay duoc du 10000 mau(100s) thi
        a1
        a2
        c1
        c2
